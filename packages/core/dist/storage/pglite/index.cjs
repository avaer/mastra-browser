...
var _PGliteStore = class _PGliteStore extends chunkI3R2VONK_cjs.MastraStorage {
  constructor({ config }) {
  ...
  async initClient(config) {
    ...
    try {
      const client = await pglite.PGlite.create(url, {
  ...
  getPrimaryKeys(tableName) {
    switch (tableName) {
      case chunkJOQIBQ7H_cjs.TABLE_THREADS:
      case chunkJOQIBQ7H_cjs.TABLE_MESSAGES:
      case chunkJOQIBQ7H_cjs.TABLE_TRACES:
        return '"id"';
      case chunkJOQIBQ7H_cjs.TABLE_WORKFLOW_SNAPSHOT:
  ...
  async getThreadsByResourceId({ resourceId }) {
    const client = await this.getClient();
    const result = await client.query(
      `SELECT * FROM ${chunkJOQIBQ7H_cjs.TABLE_THREADS} WHERE "resourceId" = $1`,
      [resourceId]
    );
  ...
  async updateThread({
    ...
    const client = await this.getClient();
    await client.query(
      `UPDATE ${chunkJOQIBQ7H_cjs.TABLE_THREADS} SET title = $1, metadata = $2 WHERE id = $3`,
  ...
  async deleteThread({ threadId }) {
    const client = await this.getClient();
    await client.query(
      `DELETE FROM ${chunkJOQIBQ7H_cjs.TABLE_THREADS} WHERE id = $1`,
      [threadId]
    );
    await client.query(
      `DELETE FROM ${chunkJOQIBQ7H_cjs.TABLE_MESSAGES} WHERE thread_id = $1`,
      [threadId]
    );
  }
  parseRow(row) {
    ...
    try {
      ...
      await client.transaction(async (tx) => {
        for (const message of messages) {
          ...
          await tx.query(
            `INSERT INTO ${chunkJOQIBQ7H_cjs.TABLE_MESSAGES} (id, thread_id, content, role, type, "createdAt") 
              VALUES ($1, $2, $3, $4, $5, $6)
  ...
  async getEvalsByAgentName(agentName, type) {
    try {
      const client = await this.getClient();
      const baseQuery = `SELECT * FROM ${chunkJOQIBQ7H_cjs.TABLE_EVALS} WHERE agent_name = $1`;
      let typeCondition = "";
      if (type === "test") {
  ...
  async getTraces({
    ...
    const client = await this.getClient();
    const result = await client.query(
      `SELECT * FROM ${chunkJOQIBQ7H_cjs.TABLE_TRACES} ${whereClause} ORDER BY "startTime" DESC LIMIT $${args.length - 1} OFFSET $${args.length}`,
      args
    );
    if (!result.rows) {
      return [];
...
chunk7D636BPD_cjs.__name(_PGliteStore, "PGliteStore");
var PGliteStore = _PGliteStore;

exports.DefaultStorage = PGliteStore;
exports.PGliteStore = PGliteStore;
