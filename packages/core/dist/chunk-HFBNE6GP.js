...
var _WorkflowInstance = class _WorkflowInstance {
  ...
  async runMachine(parentStepId, input) {
    ...
    const results = await Promise.all(
      subscriberKeys.map(async (key) => {
        if (!__privateGet(this, _stepSubscriberGraph)[key] || !this.isCompoundDependencyMet(key)) {
          return;
        }
        __privateMethod(this, _WorkflowInstance_instances, initializeCompoundDependencies_fn).call(this);
        const machine = new Machine({
          ...
          runId: this.runId,
          steps: __privateGet(this, _steps2),
          stepGraph: __privateGet(this, _stepSubscriberGraph)[key],
          executionSpan: __privateGet(this, _executionSpan2),
          startStepId: parentStepId
        });
        machine.on("state-update", stateUpdateHandler);
        __privateGet(this, _machines)[parentStepId] = machine;
  ...
  async suspend(stepId, machine) {
    __privateGet(this, _suspendedMachines)[stepId] = machine;
  }
  /**
   * Persists the workflow state to the database
   */
  async persistWorkflowSnapshot() {
    const existingSnapshot = await __privateGet(this, _mastra2)?.storage?.loadWorkflowSnapshot({
      workflowName: this.name,
      runId: __privateGet(this, _runId2)
    });
    const machineSnapshots = {};
    for (const [stepId, machine] of Object.entries(__privateGet(this, _machines))) {
    ...
    const suspendedSteps = Object.entries(__privateGet(this, _suspendedMachines)).reduce(
      (acc, [stepId, machine]) => {
    ...
    if (!snapshot && existingSnapshot) {
      ...
      await __privateGet(this, _mastra2)?.storage?.persistWorkflowSnapshot({
        workflowName: this.name,
        runId: __privateGet(this, _runId2),
      ...
      this.logger.debug("Snapshot cannot be persisted. No snapshot received.", { runId: __privateGet(this, _runId2) });
      return;
    }
  ...
  async getState() {
    const storedSnapshot = await __privateGet(this, _mastra2)?.storage?.loadWorkflowSnapshot({
      workflowName: this.name,
      runId: this.runId
    });
    ...
    const currentSnapshot = Object.entries(__privateGet(this, _machines)).reduce(
      (acc, [stepId, machine]) => {
  ...
  async _resume({ stepId, context: resumeContext }) {
    const snapshot = await __privateMethod(this, _WorkflowInstance_instances, loadWorkflowSnapshot_fn).call(this, this.runId);
    if (!snapshot) {
    ...
    if (parsedSnapshot.children) {
      Object.entries(parsedSnapshot.children).forEach(([_childId, child]) => {
        if (child.snapshot?.input?.stepNode) {
          const stepDef = __privateMethod(this, _WorkflowInstance_instances, makeStepDef_fn).call(this, child.snapshot.input.stepNode.step.id);
    ...
    if (parsedSnapshot.context?.attempts) {
      parsedSnapshot.context.attempts[stepId] = __privateGet(this, _steps2)[stepId]?.retryConfig?.attempts || __privateGet(this, _retryConfig2)?.attempts || 0;
    }
...
loadWorkflowSnapshot_fn = /* @__PURE__ */ __name(async function(runId) {
  if (!__privateGet(this, _mastra2)?.storage) {
    this.logger.debug("Snapshot cannot be loaded. Mastra engine is not initialized", { runId });
    return;
  }
  await this.persistWorkflowSnapshot();
  return __privateGet(this, _mastra2).getStorage()?.loadWorkflowSnapshot({ runId, workflowName: this.name });
}, "#loadWorkflowSnapshot");
initializeCompoundDependencies_fn = /* @__PURE__ */ __name(function() {
  Object.keys(__privateGet(this, _stepSubscriberGraph)).forEach((stepKey) => {
    if (__privateMethod(this, _WorkflowInstance_instances, isCompoundKey_fn).call(this, stepKey)) {
      const requiredSteps = stepKey.split("&&");
      __privateGet(this, _compoundDependencies)[stepKey] = requiredSteps.reduce(
        (acc, step) => {
          acc[step] = false;
          return acc;
        },
...
makeStepDef_fn = /* @__PURE__ */ __name(function(stepId) {
  const executeStep = /* @__PURE__ */ __name((handler2, spanName, attributes) => {
    return async (data) => {
      return await context.with(trace.setSpan(context.active(), __privateGet(this, _executionSpan2)), async () => {
        if (__privateGet(this, _mastra2)?.getTelemetry()) {
          return __privateGet(this, _mastra2).getTelemetry()?.traceMethod(handler2, {
            spanName,
            attributes
          })(data);
        } else {
  ...
  const handler = /* @__PURE__ */ __name(async ({ context, ...rest }) => {
    const targetStep = __privateGet(this, _steps2)[stepId];
    if (!targetStep) throw new Error(`Step not found`);
    const { payload = {}, execute = /* @__PURE__ */ __name(async () => {
    }, "execute") } = targetStep;
    const mergedData = {
      ...payload,
      ...context
    };
    const finalAction = __privateGet(this, _mastra2)?.getTelemetry() ? executeStep(execute, `workflow.${this.name}.action.${stepId}`, {
      componentName: this.name,
      runId: rest.runId
    }) : execute;
    return finalAction ? await finalAction({ context: mergedData, ...rest }) : {};
  }, "handler");
  const finalHandler = /* @__PURE__ */ __name(({ context, ...rest }) => {
    if (__privateGet(this, _executionSpan2)) {
...
isCompoundKey_fn = /* @__PURE__ */ __name(function(key) {
  return key.includes("&&");
}, "#isCompoundKey");
__name(_WorkflowInstance, "WorkflowInstance");
...
var _Workflow = class _Workflow extends MastraBase {
  ...
  constructor({
    ...
    __publicField(this, "resultSchema");
    __publicField(this, "resultMapping");
    __publicField(this, "events");
    __privateAdd(this, _retryConfig3);
    __privateAdd(this, _mastra3);
    __privateAdd(this, _runs, /* @__PURE__ */ new Map());
    __privateAdd(this, _isNested, false);
    __privateAdd(this, _onStepTransition2, /* @__PURE__ */ new Set());
    // registers stepIds on `after` calls
    __privateAdd(this, _afterStepStack, []);
    __privateAdd(this, _lastStepStack, []);
    __privateAdd(this, _ifStack, []);
    __privateAdd(this, _stepGraph3, { initial: [] });
    __privateAdd(this, _serializedStepGraph, { initial: [] });
    __privateAdd(this, _stepSubscriberGraph2, {});
    __privateAdd(this, _serializedStepSubscriberGraph, {});
    __privateAdd(this, _steps3, {});
    this.name = name;
    __privateSet(this, _retryConfig3, retryConfig);
    ...
    if (mastra) {
      ...
      __privateSet(this, _mastra3, mastra);
    }
  }
  step(next, config) {
    ...
    const step = isWorkflow(next) ? (
      // @ts-ignore
      workflowToStep(next, { mastra: __privateGet(this, _mastra3) })
    ) : next;
    ...
    const graphEntry = {
      step,
      config: {
        ...__privateMethod(this, _Workflow_instances, makeStepDef_fn2).call(this, stepKey),
        ...config,
    ...
    __privateGet(this, _steps3)[stepKey] = step;
    const parentStepKey = __privateGet(this, _afterStepStack)[__privateGet(this, _afterStepStack).length - 1];
    const stepGraph = __privateGet(this, _stepSubscriberGraph2)[parentStepKey || ""];
    const serializedStepGraph = __privateGet(this, _serializedStepSubscriberGraph)[parentStepKey || ""];
    if (parentStepKey && stepGraph) {
      ...
      if (!__privateGet(this, _stepGraph3)[stepKey]) __privateGet(this, _stepGraph3)[stepKey] = [];
      __privateGet(this, _stepGraph3).initial.push(graphEntry);
      __privateGet(this, _serializedStepGraph).initial.push(graphEntry);
    }
  ...
  then(next, config) {
    if (Array.isArray(next)) {
      const lastStep = __privateGet(this, _steps3)[__privateGet(this, _lastStepStack)[__privateGet(this, _lastStepStack).length - 1] ?? ""];
      if (!lastStep) {
      ...
      const nextSteps = next.map((step2) => {
        if (isWorkflow(step2)) {
          return workflowToStep(step2, { mastra: __privateGet(this, _mastra3) });
        }
        return step2;
      });
      nextSteps.forEach((step2) => this.step(step2, config));
    ...
    const step = isWorkflow(next) ? workflowToStep(next, { mastra: __privateGet(this, _mastra3) }) : next;
    const stepKey = __privateMethod(this, _Workflow_instances, makeStepKey_fn).call(this, step);
    const when = config?.["#internal"]?.when || config?.when;
    const graphEntry = {
      step,
    ...
    if (parentStepKey && stepGraph && stepGraph[lastStepKey]) {
      ...
      if (!__privateGet(this, _stepGraph3)[lastStepKey]) __privateGet(this, _stepGraph3)[lastStepKey] = [];
      if (!__privateGet(this, _serializedStepGraph)[lastStepKey]) __privateGet(this, _serializedStepGraph)[lastStepKey] = [];
      __privateGet(this, _stepGraph3)[lastStepKey].push(graphEntry);
      __privateGet(this, _serializedStepGraph)[lastStepKey].push(graphEntry);
    }
    return this;
  }
  loop(applyOperator, condition, fallbackStep, loopType) {
    const lastStepKey = __privateGet(this, _lastStepStack)[__privateGet(this, _lastStepStack).length - 1];
    if (!lastStepKey) return this;
    const fallbackStepKey = __privateMethod(this, _Workflow_instances, makeStepKey_fn).call(this, fallbackStep);
    __privateGet(this, _steps3)[fallbackStepKey] = fallbackStep;
    ...
    __privateGet(this, _steps3)[checkStepKey] = checkStep;
    ...
    this.after(checkStep).step(fallbackStep, {
      when: /* @__PURE__ */ __name(async ({ context }) => {
  ...
  while(condition, fallbackStep) {
    const applyOperator = /* @__PURE__ */ __name((operator, value, target) => {
      switch (operator) {
        case "$eq":
  ...
  if(condition, ifStep, elseStep) {
    ...
    this.after(lastStep);
    if (ifStep) {
      const _ifStep = isWorkflow(ifStep) ? workflowToStep(ifStep, { mastra: __privateGet(this, _mastra3) }) : ifStep;
      this.step(_ifStep, {
        when: condition
      });
      if (elseStep) {
        const _elseStep = isWorkflow(elseStep) ? workflowToStep(elseStep, { mastra: __privateGet(this, _mastra3) }) : elseStep;
        this.step(_elseStep, {
    ...
    __privateGet(this, _ifStack).push({ condition, elseStepKey, condStep: lastStep });
    return this;
  }
  else() {
    const activeCondition = __privateGet(this, _ifStack).pop();
    if (!activeCondition) {
  ...
  after(steps) {
    ...
    const stepKeys = stepsArray.map((step) => __privateMethod(this, _Workflow_instances, makeStepKey_fn).call(this, step));
    const compoundKey = stepKeys.join("&&");
    __privateGet(this, _afterStepStack).push(compoundKey);
    if (!__privateGet(this, _stepSubscriberGraph2)[compoundKey]) {
      __privateGet(this, _stepSubscriberGraph2)[compoundKey] = { initial: [] };
      __privateGet(this, _serializedStepSubscriberGraph)[compoundKey] = { initial: [] };
    }
    return this;
  }
  afterEvent(eventName) {
    ...
    const eventStep = new Step({
      id: eventStepKey,
      execute: /* @__PURE__ */ __name(async ({ context, suspend }) => {
  ...
  createRun({
    ...
    const run = new WorkflowInstance({
      ...
      retryConfig: __privateGet(this, _retryConfig3),
      steps: __privateGet(this, _steps3),
      runId,
      stepGraph: __privateGet(this, _stepGraph3),
      stepSubscriberGraph: __privateGet(this, _stepSubscriberGraph2),
      onStepTransition: __privateGet(this, _onStepTransition2),
      resultMapping: this.resultMapping,
      onFinish: /* @__PURE__ */ __name(() => {
        __privateGet(this, _runs).delete(run.runId);
      }, "onFinish"),
      events
    });
    __privateGet(this, _runs).set(run.runId, run);
    return {
   ...
   */
  getRun(runId) {
    return __privateGet(this, _runs).get(runId);
  }
  /**
  ...
  getExecutionSpan(runId) {
    return __privateGet(this, _runs).get(runId)?.executionSpan;
  }
  async getState(runId) {
    const run = __privateGet(this, _runs).get(runId);
    if (run) {
      return run.getState();
    }
    const storedSnapshot = await __privateGet(this, _mastra3)?.storage?.loadWorkflowSnapshot({
      runId,
      workflowName: this.name
    });
    if (storedSnapshot) {
  ...
  async resume({
    ...
    const activeRun = __privateGet(this, _runs).get(runId);
    if (activeRun) {
  ...
  watch(onTransition) {
    ...
    __privateGet(this, _onStepTransition2).add(onTransition);
    return () => {
      __privateGet(this, _onStepTransition2).delete(onTransition);
    };
  }
  ...
  get stepGraph() {
    return __privateGet(this, _stepGraph3);
  }
  get stepSubscriberGraph() {
    return __privateGet(this, _stepSubscriberGraph2);
  }
  get serializedStepGraph() {
    return __privateGet(this, _serializedStepGraph);
  }
  get serializedStepSubscriberGraph() {
    return __privateGet(this, _serializedStepSubscriberGraph);
  }
  get steps() {
    return __privateGet(this, _steps3);
  }
  setNested(isNested) {
    __privateSet(this, _isNested, isNested);
  }
  get isNested() {
    return __privateGet(this, _isNested);
  }
  toStep() {
    const x = workflowToStep(this, { mastra: __privateGet(this, _mastra3) });
    return new Step(x);
  }
};
...
makeStepDef_fn2 = /* @__PURE__ */ __name(function(stepId) {
  const executeStep = /* @__PURE__ */ __name((handler2, spanName, attributes) => {
    return async (data) => {
  ...
  const handler = /* @__PURE__ */ __name(async ({ context, ...rest }) => {
    const targetStep = __privateGet(this, _steps3)[stepId];
    if (!targetStep) throw new Error(`Step not found`);
    const { payload = {}, execute = /* @__PURE__ */ __name(async () => {
    }, "execute") } = targetStep;
...
//# sourceMappingURL=chunk-HFBNE6GP.js.map
//# sourceMappingURL=chunk-HFBNE6GP.js.map