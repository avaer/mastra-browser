...
describe('PGliteStore path rewriting', () => {
  let tmpDir: string;
  ...
  beforeEach(async () => {
    try {
      tmpDir = await mkdtemp(join(tmpdir(), 'mastra-test-pglite-'));
    } catch {
      // ignore
    }
  ...
  describe('Database file creation', () => {
    it('should create db file in project root when running from project root', async () => {
      const store = new PGliteStore({ config: { dataDir: 'file://test.db' } });
      await store.init();
    ...
    it('should create db file in .mastra directory when explicitly specified', async () => {
      ...
      const store = new PGliteStore({ config: { dataDir: 'file://.mastra/test.db' } });
      await store.init();
  ...
  describe('Database consistency across working directories', () => {
    it('should maintain single database file and data consistency when switching working directories', async () => {
      ...
      const store1 = new PGliteStore({ config: { dataDir: 'file://test.db' } });
      await store1.init();
      ...
      const store2 = new PGliteStore({ config: { dataDir: 'file://test.db' } });
      await store2.init();
